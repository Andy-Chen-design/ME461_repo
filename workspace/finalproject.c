...
float accelzBalancePoint = -.66; //-.76
int16 IMU_data[9];
uint16_t temp=0;
int16_t doneCal = 0;
float tilt_value = 0;
...
float Kp_front_wall = -0.5 / 304.8;
float Kp_right_wall = -1.5 / 304.8;
float front_turn_velocity = 0.4;
float forward_velocity = 1.5;
float ref_right_wall = 260;
...
uint32_t front_dis = 0;
uint32_t Rightfront_dis = 0;
...
void main(void)
{
    ...
    while(1)
    {
        if (UARTPrint == 1 ) {
            ...
            serial_printf(&SerialA,"Rightfront_dis %ld front_dis %ld",Rightfront_dis,front_dis);
...
__interrupt void SWI_isr(void) {
    ...
    //    int temp = 0;
    //    temp = xy_control(&Segbot_refSpeed, &turnref, 10, xR, yR, -3, -3, phiR, 1, 0.5);
    //    if (temp == 1) {
    //        Segbot_refSpeed = 0;
    //        turnref = 0;
    //    }

    // start obstacle avoidance and wall following

    // inside SWI1 before PI speed control
    switch (right_wall_follow_state) {
    case 1:
        //Left Turn
        //front_dis = 03 sensor reading
//        turnref = Kp_front_wall*(1300 - front_dis);
        turnrate = -0.8;
        turnref = turnref_1 + 0.004*(turnrate + turnrate_1)/2;
        Segbot_refSpeed = front_turn_velocity;
        if (front_dis > left_turn_Stop_threshold && Rightfront_dis > 200) {
    ...
    case 2:
        //Right Wall Follow
        //Rightfront_dis = 01 sensor reading
        turnrate = Kp_right_wall*(ref_right_wall - Rightfront_dis);
        turnref = turnref_1 + 0.004*(turnrate + turnrate_1)/2;
        Segbot_refSpeed = forward_velocity;
        if (front_dis < left_turn_Start_threshold) {
    ...
    if((numSWIcalls%13) == 0) { // change to the counter variable of you selected 4ms. timer
        ...
        DataToLabView.floatData[3] = temp;
        DataToLabView.floatData[4] = turnref;
        DataToLabView.floatData[5] = Segbot_refSpeed;
...
__interrupt void can_isr(void)
{
    ...
    else if(status == RX_MSG_OBJ_ID_1)
    {
        ...
        Rightfront_dis = 256*dis_raw_1[1] + dis_raw_1[0];

        measure_status_1 = rxMsgData[2];

        if (measure_status_1 == 2){
            Rightfront_dis = 1000;
        }

        if (Rightfront_dis > 400){
            Rightfront_dis = 400;
        }
    ...
    else if(status == RX_MSG_OBJ_ID_2)
    {
        ...
        front_dis = 256*dis_raw_3[1] + dis_raw_3[0];

        measure_status_3 = rxMsgData[2];

        if (measure_status_3 == 2){
            front_dis = 1000;
        }
